...
import { COMPANY_CARDS, renderMarkdown, safeUrl, socialUrl } from '@/lib/field-options'
import type {
  ActivityRow,
  CompanyAddress,
  CompanyRow,
...
import { Avatar, DealStatusBadge, LifecycleBadge, PageHeader, Section } from '@/components/ui'
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Link href={`/contacts/new?company_id=${id}`} className="btn-secondary">
              New contact
            </Link>
      ...
      <div className="mb-5">
        <Section title={COMPANY_CARDS[0].label}>
          <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Owner">{userName(company.owner_id) ?? <Empty />}</Field>
            ...
            <Field label="Specialty market" wide>
              <OptionBadges values={company.specialty_market} options={optionsFor('specialty_market')} />
            </Field>
          ...
          <Section title="Record history">
            <dl className="space-y-3">
              <Field label="Created by">
                <span className="block">{userName(company.created_by) ?? 'Unknown'}</span>
                <span className="text-xs text-slate-500">{formatDateTime(company.created_at)}</span>
              </Field>
              <Field label="Updated by">
                <span className="block">{userName(company.updated_by) ?? 'Unknown'}</span>
                <span className="text-xs text-slate-500">{formatDateTime(company.updated_at)}</span>
              </Field>
            </dl>
          </Section>
        </div>
